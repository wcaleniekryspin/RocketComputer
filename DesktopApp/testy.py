...
class RocketMonitorApp:
    ...
    def setup_custom_graphs(self, parent):
        ...
        row1.pack(fill=tk.BOTH, expand=False, pady=5)
        ...
        row2.pack(fill=tk.BOTH, expand=False, pady=5)
    ...
    def setup_rocket_canvas(self, parent):
        ...
        rocket_frame.pack(fill=tk.BOTH, expand=False, pady=5)
    ...
    def setup_controls(self, parent):
        ...
        self._controls_window_id = canvas.create_window((0, 0), window=controls_frame, anchor=tk.NW)
        ...
        def update_scrollregion(event=None):
            ...
            try:
                if controls_frame.winfo_reqwidth() < canvas.winfo_width():
                    # use stored window id instead of hard-coded 1
                    canvas.itemconfig(self._controls_window_id, width=canvas.winfo_width())
            except Exception:
                pass

        controls_frame.bind("<Configure>", update_scrollregion)
        canvas.bind("<Configure>", lambda e: canvas.itemconfig(self._controls_window_id, width=max(
    ...
    def log(self, message, level="info"):
        """Add message to log with timestamp (robust tag handling and trimming)"""
        ...
        # Compute indices for the just-inserted line safely
        try:
            line_start = self.log_text.index("end-1c linestart")
            line_end = self.log_text.index("end-1c")
        except Exception:
            # fallback to a safe default if index computation fails
            line_start = "end-1c"
            line_end = "end-1c"

        # Apply tag based on level; guard tag_add so logging never raises
        try:
            if level == "error":
                ...
                self.log_text.tag_add("sent", line_start, line_end)
            elif level == "received":
                self.log_text.tag_add("received", line_start, line_end)
        except tk.TclError:
            # ignore tag errors to keep UI responsive
            pass

        # Auto-scroll
        self.log_text.see(tk.END)

        # Limit log size to last 100 lines using reliable line count
        try:
            line_count = int(self.log_text.index('end-1c').split('.')[0])
            if line_count > 100:
                # delete oldest lines and keep last 100
                delete_to = f"{line_count - 100 + 1}.0"
    ...
    def update_value_labels(self, accel_x, accel_y, accel_z, lsm_temp, bmp_temp,
        ...
        if hasattr(self, 'value_labels') and isinstance(self.value_labels, dict):
            if 'accel_x' in self.value_labels:
                self.value_labels['accel_x'].config(text=f"{accel_x}")
            if 'accel_y' in self.value_labels:
                self.value_labels['accel_y'].config(text=f"{accel_y}")
            if 'accel_z' in self.value_labels:
                self.value_labels['accel_z'].config(text=f"{accel_z}")
            if 'lsm_temp' in self.value_labels:
                self.value_labels['lsm_temp'].config(text=f"{lsm_temp}")
            if 'bmp_temp' in self.value_labels:
                self.value_labels['bmp_temp'].config(text=f"{bmp_temp}")
            if 'bmp_press' in self.value_labels:
                self.value_labels['bmp_press'].config(text=f"{bmp_press}")
            if 'bmp_alti' in self.value_labels:
                self.value_labels['bmp_alti'].config(text=f"{bmp_alti}")
            if 'battery' in self.value_labels:
                self.value_labels['battery'].config(text=f"{batt_percent:.1f}%")

            if 'status_value' in self.value_labels:
                self.value_labels['status_value'].config(text=f"{status_val}")

            if 'error_value' in self.value_labels:
                self.value_labels['error_value'].config(text=f"{error_val}")
